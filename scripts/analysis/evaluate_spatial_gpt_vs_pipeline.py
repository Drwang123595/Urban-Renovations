...
from scripts.analysis.spatial_blind_label_common import (  # noqa: E402
    area_match_type,
    binary_metrics,
    normalize_scale,
...
def load_and_align(labels_path: Path, pipeline_path: Path) -> pd.DataFrame:
    ...
    if "row_index" not in labels.columns:
        labels = labels.reset_index(names="row_index")
    labels["row_index"] = labels["row_index"].astype(int)
    pipeline = pipeline.reset_index(names="row_index")
    merged = labels.merge(
        pipeline,
        on="row_index",
        how="left",
...
def main() -> int:
    ...
    merged = load_and_align(args.labels, args.pipeline)

...
DEFAULT_TAG = "20260417_unknown_recovery"
...
def run_logged(label: str, command: list[str], *, log_file: Path, dry_run: bool = False) -> None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--no-gate", action="store_true", help="Run artifacts without enforcing stable gates")
...
def main() -> None:
    ...
    if args.skip_classification:
        ...
        run_logged("classification", commands["classification"], log_file=paths.log_file, dry_run=args.dry_run)
    run_logged("evaluation", commands["evaluation"], log_file=paths.log_file, dry_run=args.dry_run)
    run_logged("unknown_review", commands["unknown_review"], log_file=paths.log_file, dry_run=args.dry_run)

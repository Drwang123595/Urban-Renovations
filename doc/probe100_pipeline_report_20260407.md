# Probe100 管线测试报告（2026-04-07）

## 1. 测试目标

- 打通当前城市更新识别整套流程。
- 选取 100 篇有标签样本进行端到端测试。
- 在不改提示词和主题体系的前提下，优先通过第一阶段轻量负筛与 `hybrid` 路由优化，把最终 `hybrid` 准确率提升到 `>= 85%`。
...
目的:
- 把明确的农村 / 非城市场景样本更早拦截
- 避免这类样本进入 `stage2/stage3` 后被误判为城市更新
...
- 新增更严格的 LLM 介入门槛:
  ...
  - 其他城市更新正类桶默认直接信任 `stage2 classifier`，不再送入 LLM 重新判定

目的:
- 减少 LLM 对已经较稳定的城市更新正例进行错误翻转
- 保留其在不确定非城市场景样本和部分 `U4` 社会影响类样本中的修正价值
...
新增/调整:
- `rural gentrification` 的 `Stage1` 排除测试
- 低置信城市更新样本 + 边际坍缩时不再进入 LLM
- `U4` 低置信但 margin 仍稳定时允许进 LLM
- 噪声较高的 `U2` 样本默认回退到 `stage2 classifier`
- BERTopic 离群样本测试改为非城市场景
...
2. `Stage2` 仍是主要剩余误差来源
   - `stage2_only_error = 9`
   - 说明下一轮如果要继续提升到 `88%-90%`，重点应放在主题体系或训练样本，而不是继续加重 `Stage1`

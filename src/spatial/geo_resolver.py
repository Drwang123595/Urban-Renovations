...
class GeoResolver:
    ...
    def _load_builtin_overlay(self) -> None:
        entries = [
            ...
            GeoEntity("es-madrid", "Madrid", country_code="ES", admin_level="PPL", feature_code="PPLC", mapped_scale_level=SCALE_LEVELS["7"], source="builtin_city"),
            GeoEntity("es-valencia", "Valencia", country_code="ES", admin_level="PPL", feature_code="PPLA", mapped_scale_level=SCALE_LEVELS["7"], source="builtin_city"),
            GeoEntity("es-bilbao", "Bilbao", country_code="ES", admin_level="PPL", feature_code="PPLA2", mapped_scale_level=SCALE_LEVELS["7"], source="builtin_city"),
            GeoEntity("es-sevilla", "Sevilla", country_code="ES", admin_level="PPL", feature_code="PPLA", mapped_scale_level=SCALE_LEVELS["7"], source="builtin_city"),
            GeoEntity("br-sao-paulo-city", "Sao Paulo", country_code="BR", admin_level="PPL", feature_code="PPLA", mapped_scale_level=SCALE_LEVELS["7"], source="builtin_city"),
            GeoEntity("mx-mexico-city", "Mexico City", country_code="MX", admin_level="PPL", feature_code="PPLC", mapped_scale_level=SCALE_LEVELS["7"], source="builtin_city"),
            GeoEntity("et-addis-ababa-city", "Addis Ababa", country_code="ET", admin_level="PPL", feature_code="PPLC", mapped_scale_level=SCALE_LEVELS["7"], source="builtin_city"),
            GeoEntity("us-new-orleans", "New Orleans", country_code="US", admin_level="PPL", feature_code="PPLA2", mapped_scale_level=SCALE_LEVELS["7"], source="builtin_city"),
        ...
        aliases = {
            ...
            "br-sao-paulo-city": ["São Paulo", "Sao Paulo City"],
            "mx-mexico-city": ["Ciudad de Mexico", "Ciudad de México"],
            "et-addis-ababa-city": ["Addis Abeba"],
            "us-new-orleans": ["New Orleans metropolitan area"],
            "pl-poznan": ["Poznań"],
        }
    ...
    def resolve(self, area: str, llm_scale_level: Optional[str] = None) -> GeoResolution:
        ...
        if direct:
            scale_override = llm_scale_level if self._should_preserve_metropolitan_scale(cleaned, llm_scale_level) else None
            scale_source = "mapping_override_llm" if scale_override else None
    ...
    def _split_area(self, area: str) -> List[str]:
        if re.search(r"\b(?:in|within|of)\b", area, flags=re.IGNORECASE) and not re.search(
            r",\s*[^,]+(?:\s*,|\s+\band\b)",
            area,
            flags=re.IGNORECASE,
        ):
    ...
    def _should_preserve_metropolitan_scale(self, area: str, llm_scale_level: Optional[str]) -> bool:
        return bool(
            llm_scale_level
            and "metropolitan area" in self.normalize(area)
            and self._scale_number(llm_scale_level) <= 6
        )

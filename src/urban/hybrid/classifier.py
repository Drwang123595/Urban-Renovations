...
from ..topic_model.local_classifier import TopicPrediction, UrbanTopicClassifier
...
UNKNOWN_URBAN_RECOVERY_BLOCK_TERMS = {
    "tenure security",
...
NONURBAN_PROMOTION_BLOCK_TERMS = (
    "rural",
    "agricultur",
    "farmland",
    "peri-urban",
...
BROWNFIELD_NONURBAN_BLOCK_TERMS = (
...
class UrbanHybridClassifier:
    ...
    def _nonurban_promotion_block_reason(
        self,
        *,
        record: UrbanMetadataRecord,
        base: Dict[str, Any],
        ...
        if route_reason in {"rural_nonurban", "math_term_misuse"}:
            return f"route={route_reason}"

        risk_tags = self._extract_stage1_risk_tags(base)
        if RISK_GREENFIELD_EXPANSION in risk_tags:
            return f"risk_tag={RISK_GREENFIELD_EXPANSION}"

        text = f"{record.title} {record.abstract}".lower()
        if final_topic == "N9":
            return "final_topic=N9"
        for term in NONURBAN_PROMOTION_BLOCK_TERMS:
            if term in text:
                return f"text={term}"
        if "brownfield" in text:
            for term in BROWNFIELD_NONURBAN_BLOCK_TERMS:
                if term in text:
                    return f"brownfield_nonurban={term}"
        return ""

    def _has_renewal_anchor_signal(self, *, record: UrbanMetadataRecord) -> bool:
        return bool(
            self._extract_core_anchor_hits(title=record.title, abstract=record.abstract)
            or self._extract_broad_anchor_hits(title=record.title, abstract=record.abstract)
        )

    def _unknown_urban_recovery_block_reason(self, *, record: UrbanMetadataRecord) -> str:
        text = f"{record.title} {record.abstract}".lower()
        for term in UNKNOWN_URBAN_RECOVERY_BLOCK_TERMS:
            if term in text:
                return f"text={term}"
        return ""
    ...
    def _apply_anchor_guard(
        ...
        *,
        ...
        if block_reason:
            base["anchor_guard_action"] = "blocked_nonurban_risk"
            base["anchor_guard_reason"] = block_reason
            decision_reason = f"{decision_reason};anchor_guard_blocked:{block_reason}".strip(";")
            return final_topic, decision_source, decision_reason, confidence, review_flag, review_reason
    ...
    def _open_set_urban_evidence(
        ...
        *,
        ...
        if block_reason:
            return False, "nonurban_promotion_blocked", block_reason
    ...
    def _apply_open_set_topic(
        ...
        *,
        ...
        allow_nonurban_review_recovery = (
            topic_group == "nonurban"
            and str(decision_source or "").strip() in {"uncertain_nonurban_review", "anchor_guard_review"}
        )
        if (
            topic_group in {"urban", "nonurban"}
            and final_topic not in {OPEN_SET_URBAN_LABEL, OPEN_SET_NONURBAN_LABEL}
            and not allow_nonurban_review_recovery
        ):
    ...
    def _resolve_unknown_with_hints(
        ...
        *,
        ...
        urban_recovery_block_reason = self._unknown_urban_recovery_block_reason(record=record)
        urban_recovery_blocked = bool(urban_recovery_block_reason)
    ...
    def _reconcile_final_review_signal(
        ...
        *,
        ...
        for item in old_reasons:
            if item in manual_review_reasons:
                reasons.append(item)
    ...
    def _summarize_decision_explanation(
        ...
        *,
        ...
        explanation = (
            f"final={final_label_display}; score={score:.4f}{comparator}threshold={threshold:.4f}; "

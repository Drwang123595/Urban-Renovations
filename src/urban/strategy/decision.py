...
class StableUrbanDecisionEngine:
    ...
    def decide(self, evidence: EvidenceBundle) -> StableDecisionResult:
        ...
        if self._has_background_or_method_only_subject(evidence):
            return self._result(
                "0",
                evidence.current_topic,
    ...
    def _has_strong_positive_rule(self, evidence: EvidenceBundle) -> bool:
        ...
        return bool(
            ...
            and self._has_main_subject_support(evidence)
        )
    ...
    def _has_main_subject_support(self, evidence: EvidenceBundle) -> bool:
        return self._main_subject_evidence(evidence) == "core_object_and_action_in_title_or_abstract"

    def _has_background_or_method_only_subject(self, evidence: EvidenceBundle) -> bool:
        return self._main_subject_evidence(evidence) == "background_or_method_only"

    def _main_subject_evidence(self, evidence: EvidenceBundle) -> str:
    ...
    def _result(
        ...
        *,
        ...
        return StableDecisionResult(
            ...
            core_object_evidence=self._core_object_summary(evidence),
            renewal_action_evidence=self._renewal_action_summary(evidence),
            main_subject_evidence=self._main_subject_evidence(evidence),
            risk_evidence=self._risk_summary(evidence),
            auxiliary_evidence=self._auxiliary_summary(evidence),
        )

    def _core_object_summary(self, evidence: EvidenceBundle) -> str:
    ...
    def _renewal_action_summary(self, evidence: EvidenceBundle) -> str:
    ...
    def _risk_summary(self, evidence: EvidenceBundle) -> str:
        parts: list[str] = []
        if evidence.rule.hard_exclusion_reason:
    ...
    def _auxiliary_summary(self, evidence: EvidenceBundle) -> str:
        parts: list[str] = []
        ...
        cluster_id = evidence.cluster.cluster_id
        if cluster_id:
    ...
    def _negative_summary(self, evidence: EvidenceBundle) -> str:
        ...
        if evidence.rule.risk_hits:
            parts.append("risk=" + ",".join(evidence.rule.risk_hits))
        main_subject = self._main_subject_evidence(evidence)

...
class MetadataRuleFilter:
    ...
    def evaluate(self, record: UrbanMetadataRecord) -> MetadataRouteResult:
        ...
        if len(topic_rule_top3) >= 2:
            ...
            if topic_group_for_label(top1_label) != topic_group_for_label(top2_label):
                stage1_conflict_flag = 1

        review_flag_rule = int(
            topic_rule == UNKNOWN_TOPIC_LABEL
            or topic_rule_score < 4.0
            or topic_rule_margin < 2.0
            or stage1_conflict_flag == 1
        )
        review_reason_rule = ""
        if topic_rule == UNKNOWN_TOPIC_LABEL:
            review_reason_rule = "rule_unknown"
        elif stage1_conflict_flag == 1:
            review_reason_rule = "rule_group_conflict"
        elif topic_rule_margin < 2.0:
            review_reason_rule = "rule_low_margin"
        elif topic_rule_score < 4.0:
            review_reason_rule = "rule_low_score"

        reason = "uncertain_pass"
        if other_object_topic:
            reason = other_object_reason
        elif rule_high_confidence:
            reason = f"rule_topic_{topic_rule.lower()}"
        elif greenfield_hits and topic_rule == "N2":
            reason = "greenfield_expansion_signal"
        elif method_algo_hits and topic_rule == "N8":
            reason = "method_signal"
        elif materials_hits and topic_rule == "N10":
            reason = "environment_signal"

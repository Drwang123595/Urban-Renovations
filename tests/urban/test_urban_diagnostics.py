...
from src.urban.pipeline.diagnostics import (
    build_urban_diagnostics,
    build_urban_diagnostics_frame,
    build_urban_error_attribution,
)
...
def test_build_urban_error_attribution_groups_false_positive_and_false_negative_sources():
    frame = pd.DataFrame(
        [
            {
            ...
            },
            {
                ...
                "urban_flag": "0",
                "topic_final": "N3",
                "decision_source": "uncertain_nonurban_review",
    ...
    attribution = build_urban_error_attribution(frame, truth_column=Schema.IS_URBAN_RENEWAL)

    assert attribution.loc[attribution["error_type"].eq("false_positive"), "count"].sum() == 1
    assert attribution.loc[attribution["error_type"].eq("false_negative"), "count"].sum() == 1
    assert set(attribution["decision_source"]) >= {"unknown_review", "uncertain_nonurban_review"}

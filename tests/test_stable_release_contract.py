...
STABLE_OUTPUT_FILE = STABLE_OUTPUT_DIR / "urban_renewal_three_stage_hybrid_few_llm_on_20260417_unknown_recovery.xlsx"
...
STABLE_UNKNOWN_REVIEW_FILE = (
    ...
    / "20260417_unknown_recovery"
    / "reviews"
    / "Unknown_Review_hybrid_llm_on_20260417_unknown_recovery.xlsx"
)
STABLE_FILE_STEM = "urban_renewal_three_stage_hybrid_few_llm_on_20260417_unknown_recovery"
...
def test_readme_locks_current_stable_release():
    ...
    assert "runs/stable_release/20260417_unknown_recovery" in readme
...
def test_stable_release_decision_source_and_unknown_contract():
    ...
    assert set(stable_decisions["Decision Source"]) == {
        "rule_model_fusion",
        "stage1_rule",
...
def test_stable_release_output_contract_and_llm_usage():
    ...
    assert int(pd.to_numeric(output_df["llm_attempted"], errors="coerce").fillna(0).sum()) == 137
    assert set(output_df["decision_source"].fillna("")) <= {
        "rule_model_fusion",
        "stage1_rule",
        "unknown_hint_resolution",
        "unknown_review",
